...
async function renderNetwork() {
  ...
  $('#shape-form').onsubmit=async event=>{event.preventDefault();const form=new FormData(event.target),live=router.liveApplyEnabled&&confirm('Apply these CAKE limits to the live LAN and WAN interfaces now?');const result=await api('/router/apply',{method:'POST',body:JSON.stringify({interface:form.get('interface'),wanInterface:wan?.name||null,downloadMbps:+form.get('downloadMbps'),uploadMbps:+form.get('uploadMbps'),dryRun:!live})});toast(result.message);};
}
