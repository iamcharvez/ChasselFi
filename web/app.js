...
const state = { overview: null, system: null, business: null, rates: [], sessions: [], vouchers: [], transactions: [], sites: [], settings: null, coinNodes: [], operations: null };
let revenueChart = null;
let bandwidthChart = null;
let bandwidthTimer = null;
let sessionRefreshTimer = null;
let loginVisible = false;
const pageNames = { dashboard: 'Dashboard', sales: 'Sales & inventory', sessions: 'Live sessions', rates: 'Timer rates', vouchers: 'Voucher studio', 'free-time': 'Free time', 'coin-nodes': 'Coin nodes', network: 'Network status', tools: 'Site blocking', operations: 'Operations', 'portal-design': 'Portal designer', settings: 'Settings' };
...
async function renderDashboard() {
  ...
  $('#app').innerHTML = `
    ...
    <section class="stats-grid">
      ...
      <article class="stat-card"><small>Online users</small><strong id="dashboard-online-users">${o.onlineUsers}</strong><footer><span id="dashboard-waiting-users">${s.waitingUsers||0}</span> waiting · ${o.pausedUsers} paused</footer></article>
      ...
      <div>
        <section class="card">
          ...
          <div class="system-list">
            ...
            <div class="system-row"><div><span>Captive gateway</span><strong id="health-gateway" class="${s.gatewayOnline?'text-success':'text-danger'}">${s.gatewayOnline?'ONLINE':'UNAVAILABLE'}</strong></div><small id="health-clients">${s.onlineUsers} authenticated · ${s.waitingUsers||0} waiting for sign-in</small></div>
...
async function startBandwidthChart(){
  ...
  let previous=null;
  const poll=async()=>{try{const [interfaces,system]=await Promise.all([api('/network/interfaces'),api('/system')]),item=interfaces.find(entry=>entry.name.endsWith('.799'))||interfaces.find(entry=>entry.name!=='lo'&&entry.state==='up');if($('#dashboard-online-users'))$('#dashboard-online-users').textContent=system.onlineUsers;if($('#dashboard-waiting-users'))$('#dashboard-waiting-users').textContent=system.waitingUsers||0;if($('#health-gateway')){$('#health-gateway').textContent=system.gatewayOnline?'ONLINE':'UNAVAILABLE';$('#health-gateway').className=system.gatewayOnline?'text-success':'text-danger';}if($('#health-clients'))$('#health-clients').textContent=`${system.onlineUsers} authenticated · ${system.waitingUsers||0} waiting for sign-in`;if(!item)return;$('#bandwidth-interface').textContent=item.name;if(previous){const elapsed=(Date.now()-previous.at)/1000,upload=Math.max(0,(item.rxBytes-previous.rx)/elapsed),download=Math.max(0,(item.txBytes-previous.tx)/elapsed);$('#bandwidth-rx').textContent=formatRate(download);$('#bandwidth-tx').textContent=formatRate(upload);data.labels.push(new Date().toLocaleTimeString());data.datasets[0].data.push(download*8/1e6);data.datasets[1].data.push(upload*8/1e6);if(data.labels.length>40){data.labels.shift();data.datasets.forEach(set=>set.data.shift());}bandwidthChart.update('none');}previous={rx:item.rxBytes,tx:item.txBytes,at:Date.now()};}catch{}};
...
async function renderSessions() {
  ...
  const authenticated = active.filter(s=>s.gatewayAuthenticated);
  const waiting = active.filter(s=>s.status==='waiting'||(!s.gatewayAuthenticated&&s.gatewayConnected));
  $('#app').innerHTML = `${pageHead('Client command center', 'Connected users', 'Pause, extend, throttle, rename, resume, or revoke a customer from one screen.', `<button class="secondary-btn" id="refresh-sessions">↻ Refresh live data</button>`)}
    <section class="stats-grid">
      <article class="stat-card"><small>Authenticated</small><strong>${authenticated.length}</strong><footer>Confirmed by openNDS</footer></article>
      <article class="stat-card"><small>Waiting / paused</small><strong>${waiting.length} / ${active.filter(s=>s.status==='paused').length}</strong><footer>Visible before sign-in too</footer></article>
      <article class="stat-card"><small>Live download</small><strong>${formatKbps(active.reduce((a,s)=>a+(+s.downloadKbps||0),0))}</strong><footer>Gateway measurement</footer></article>
      <article class="stat-card"><small>Live upload</small><strong>${formatKbps(active.reduce((a,s)=>a+(+s.uploadKbps||0),0))}</strong><footer>Gateway measurement</footer></article>
    </section>
  ...
  sessionRefreshTimer=setTimeout(()=>{if(location.hash==='#sessions')renderSessions().catch(console.error);},2000);
}
function sessionCard(s) {
  const percent = Math.min(100, Math.max(4, s.remainingSeconds / 43200 * 100));
  const managed=s.managed!==false&&s.id;
  const warning=s.identityConflict?'IP/MAC conflict blocked':s.enforcementMismatch?'Database and gateway state differ':'';
  return `<article class="session-card ${warning?'session-warning':''}"><div class="session-top"><div class="device-icon">${s.gatewayAuthenticated?'●':'Ⅱ'}</div><div style="flex:1"><strong>${esc(s.clientName)}</strong><small style="display:block">${esc(s.ip)} · ${esc(s.mac||'MAC unavailable')}</small></div>${badge(s.gatewayState||s.status,s.gatewayAuthenticated?'':s.status==='waiting'?'orange':'red')}</div>${warning?`<p class="security-note">${esc(warning)}</p>`:''}<div class="session-time"><strong>${managed?remaining(s.remainingSeconds):'Sign-in required'}</strong><span>${managed?'remaining':'openNDS preauthenticated client'}</span><div class="mini-meter"><i style="width:${managed?percent:4}%"></i></div></div><div class="session-meta"><div><small>Live download</small><strong>${formatKbps(s.downloadKbps)}</strong></div><div><small>Live upload</small><strong>${formatKbps(s.uploadKbps)}</strong></div><div><small>Total down / up</small><strong>${formatBytes(s.downloadedBytes)} / ${formatBytes(s.uploadedBytes)}</strong></div><div><small>Policy cap</small><strong>${managed?`${s.downloadMbps} / ${s.uploadMbps} Mbps`:'Not authorized'}</strong></div><div><small>Interface</small><strong>${esc(s.clientInterface||'—')}</strong></div><div><small>Identity</small><strong>${s.identityConflict?'Blocked':'IP + MAC verified'}</strong></div></div>${managed?`<div class="session-actions"><button class="secondary-btn" data-edit-session="${s.id}">Edit / add time</button>${s.status==='online'?`<button class="secondary-btn" data-session="${s.id}" data-action="pause">Pause</button>`:`<button class="secondary-btn" data-session="${s.id}" data-action="resume">Resume</button>`}<button class="danger-outline" data-session="${s.id}" data-action="stop">Revoke</button></div>`:''}</article>`;
}
function formatKbps(kbps){const rate=Number(kbps)||0;return rate>=1000?`${(rate/1000).toFixed(2)} Mbps`:`${rate.toFixed(1)} Kbps`;}
...
async function renderVouchers() {
  [state.vouchers,state.settings,state.rates] = await Promise.all([api('/vouchers'),api('/settings'),api('/rates')]);
...
function voucherModal() {
  const rates=state.rates.filter(rate=>rate.active);
  openModal('Generate voucher batch', `<form class="modal-form needs-validation" id="voucher-form"><div class="field"><label class="form-label">Timer package</label><select class="form-control" name="rateId" required>${rates.map(rate=>`<option value="${rate.id}" data-price="${rate.price}" data-minutes="${rate.minutes}">${money(rate.price)} · ${duration(rate.minutes)} · ${rate.downloadMbps}/${rate.uploadMbps} Mbps</option>`).join('')}</select><small>The selected timer rate now controls voucher time and speed.</small></div><div class="form-grid"><div class="field"><label class="form-label">Quantity</label><input class="form-control" name="quantity" type="number" min="1" max="100" value="10" required></div><div class="field"><label class="form-label">Price each (₱)</label><input class="form-control" name="price" type="number" min="0" value="${rates[0]?.price||10}" readonly required></div><div class="field"><label class="form-label">Access time (minutes)</label><input class="form-control" name="minutes" type="number" min="1" value="${rates[0]?.minutes||120}" readonly required></div><div class="field"><label class="form-label">Expires after (days)</label><input class="form-control" name="expiresInDays" type="number" min="1" value="30"></div></div><div class="modal-actions"><button type="button" class="btn secondary-btn" data-close-modal>Cancel</button><button class="btn primary-btn">Generate codes</button></div></form>`);
  $('[name="rateId"]',$('#voucher-form')).onchange=event=>{const option=event.target.selectedOptions[0];$('[name="price"]',$('#voucher-form')).value=option.dataset.price;$('[name="minutes"]',$('#voucher-form')).value=option.dataset.minutes;};
  $('#voucher-form').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target),body={rateId:f.get('rateId'),quantity:+f.get('quantity'),price:+f.get('price'),minutes:+f.get('minutes'),expiresInDays:+f.get('expiresInDays')};const codes=await api('/vouchers/generate',{method:'POST',body:JSON.stringify(body)});closeModal();toast(`${codes.length} vouchers generated from the selected timer rate`);renderPage('vouchers');};
}
...
async function renderCoinNodes(){
  ...
  <section class="card"><div class="card-head"><div><h2>Paired hardware</h2><p>Keys are stored as hashes and shown only once during pairing</p></div></div><div class="node-grid">${state.coinNodes.map(node=>`<article class="node-card ${node.disabled?'is-disabled':''}"><div class="node-status ${node.online&&!node.disabled?'online':''}"><i></i>${node.disabled?'DISABLED':node.online?'ONLINE':'OFFLINE'}</div><div class="node-icon">◫</div><h3>${esc(node.name)}</h3><code>${esc(node.id)}</code><dl><div><dt>LAN address</dt><dd>${esc(node.clientIp||'Not connected')}</dd></div><div><dt>Firmware</dt><dd>${esc(node.firmware||'Unknown')}</dd></div><div><dt>Last seen</dt><dd>${node.lastSeenAt?dateTime(node.lastSeenAt):'Never'}</dd></div><div><dt>Accepted pulses</dt><dd>${node.acceptedPulses||0} · seq ${node.lastSequence||0}</dd></div></dl><div class="node-actions"><button class="secondary-btn" data-toggle-node="${esc(node.id)}" data-enabled="${node.disabled}">${node.disabled?'Enable':'Disable'}</button><button class="danger-outline" data-delete-node="${esc(node.id)}">Unpair</button></div></article>`).join('')||empty('No coin nodes paired','Pair hardware to generate a device ID and one-time API key.')}</div></section>
...
async function renderNetwork() {
  ...
  <div class="two-col"><section class="card"><div class="card-head"><div><h2>Per-session bandwidth</h2><p>Enforced per authenticated client by openNDS and each timer package</p></div></div>${metric('Download',`${state.settings?.downloadLimitMbps||15} Mbps`,65)}${metric('Upload',`${state.settings?.uploadLimitMbps||15} Mbps`,45)}</section><section class="card"><div class="card-head"><div><h2>Global CAKE shaping</h2><p>${router.message}</p></div>${badge(router.mode, router.liveApplyEnabled ? '' : 'orange')}</div><div class="interface-list"><div class="interface"><div><strong>tc / CAKE</strong><small>${router.tcAvailable&&router.cakeAvailable ? 'Ready' : 'Not ready'}</small></div><div><strong>openNDS</strong><small>${router.openndsAvailable ? 'Available' : 'Not detected'}</small></div><div><strong>nftables</strong><small>${router.nftAvailable ? 'Available' : 'Not detected'}</small></div><div>${badge(router.liveApplyEnabled ? 'Live-capable' : 'Preview only', router.liveApplyEnabled ? '' : 'orange')}</div></div></div><p class="security-note">CAKE controls the whole 150 Mbps-class line and provides per-host fairness. It must not be set to the 15 Mbps customer cap.</p><form id="shape-form" class="form-grid" style="margin-top:16px"><div class="field"><label class="form-label">Customer LAN</label><input class="form-control" name="interface" value="${shapingInterface}" maxlength="15" required></div><div class="field"><label class="form-label">WAN interface</label><input class="form-control" name="wanInterface" value="${wan?.name||discovery.recommendedWan||''}" maxlength="15" required></div><div class="field"><label class="form-label">Aggregate download Mbps</label><input class="form-control" name="downloadMbps" type="number" min="1" value="${state.settings?.wanDownloadMbps||142}" required></div><div class="field"><label class="form-label">Aggregate upload Mbps</label><input class="form-control" name="uploadMbps" type="number" min="1" value="${state.settings?.wanUploadMbps||142}" required></div><div class="field full"><button class="btn primary-btn w-100">${router.liveApplyEnabled?'Apply and verify CAKE':'Validate shaping plan'}</button></div></form></section></div>`;
  ...
  $('#shape-form').onsubmit=async event=>{event.preventDefault();const form=new FormData(event.target),live=router.liveApplyEnabled&&confirm('Apply these aggregate CAKE ceilings with per-host fairness to the live LAN and WAN now?');const result=await api('/router/apply',{method:'POST',body:JSON.stringify({interface:form.get('interface'),wanInterface:form.get('wanInterface')||null,downloadMbps:+form.get('downloadMbps'),uploadMbps:+form.get('uploadMbps'),dryRun:!live})});if(result.applied){state.settings.wanDownloadMbps=+form.get('downloadMbps');state.settings.wanUploadMbps=+form.get('uploadMbps');await saveSettingsObject(state.settings);}toast(result.message);};
}
...
async function renderOperations(){
  const [metrics,diagnostics,audit]=await Promise.all([api('/operations/metrics'),api('/diagnostics'),api('/audit-events?limit=150')]);
  state.operations={metrics,diagnostics,audit};
  const checks=diagnostics.checks||[];
  $('#app').innerHTML=`${pageHead('Operational intelligence','Operations','See whether access enforcement—not only the web server—is healthy.',`<button class="secondary-btn" id="run-diagnostics">↻ Refresh</button><button class="primary-btn" id="reconcile-gateway">Reconcile gateway</button>`)}
  <section class="stats-grid"><article class="stat-card"><small>Platform health</small><strong class="${diagnostics.healthy?'text-success':'text-warning'}">${diagnostics.healthy?'Healthy':'Attention'}</strong><footer>${checks.filter(check=>check.ok).length}/${checks.length||0} checks passing</footer></article><article class="stat-card"><small>Active / paused</small><strong>${metrics.activeSessions} / ${metrics.pausedSessions}</strong><footer>Gateway-accounted sessions</footer></article><article class="stat-card"><small>Revenue · 24h</small><strong>${money(metrics.revenue24h)}</strong><footer>${metrics.transactions24h} transactions</footer></article><article class="stat-card"><small>Coin nodes online</small><strong>${metrics.coinNodesOnline}</strong><footer>Heartbeat within 45 seconds</footer></article></section>
  <div class="two-col operations-layout"><section class="card"><div class="card-head"><div><h2>Enforcement checks</h2><p>Live probes from this server</p></div>${badge(diagnostics.hardwareMode)}</div><div class="health-checks">${checks.map(check=>`<details class="health-check ${check.ok?'ok':'failed'}"><summary><i></i><span><b>${esc(check.name)}</b><small>${esc(check.kind)}</small></span><strong>${check.ok?'PASS':'FAIL'}</strong></summary><pre>${esc(check.detail||'No output')}</pre></details>`).join('')||empty('Simulation mode','Install on Linux to run system service and gateway probes.')}</div></section>
  <section class="card"><div class="card-head"><div><h2>Recovery posture</h2><p>Production safeguards currently configured</p></div></div><div class="recovery-list"><div><span>Ready voucher stock</span><b>${metrics.readyVouchers}</b></div><div><span>Audit events · 24h</span><b>${metrics.auditEvents24h}</b></div><div><span>Ended sessions retained</span><b>${metrics.endedSessions}</b></div><div><span>Local coin adapter</span><b>${metrics.unpairedCoinSocketReady?'Ready':'Standby'}</b></div></div><p class="security-note">A restore now creates a pre-restore recovery snapshot and verifies SHA-256 integrity before replacing live state.</p></section></div>
  <section class="card table-card"><div class="table-tools"><div><strong>Audit trail</strong><small style="display:block">Newest security, session, hardware, gateway, and recovery events</small></div></div><div class="table-wrap">${audit.length?`<table class="table"><thead><tr><th>Time</th><th>Category</th><th>Action</th><th>Actor</th><th>Subject</th><th>Detail</th></tr></thead><tbody>${audit.map(event=>`<tr><td>${dateTime(event.timestamp)}</td><td>${badge(esc(event.category))}</td><td><strong>${esc(event.action)}</strong></td><td>${esc(event.actor)}</td><td class="mono">${esc(event.subject)}</td><td>${esc(event.detail)}</td></tr>`).join('')}</tbody></table>`:empty('No audit events yet','Production events will appear as operators and customers use the system.')}</div></section>`;
  $('#run-diagnostics').onclick=()=>renderPage('operations');
  $('#reconcile-gateway').onclick=async()=>{const result=await api('/gateway/reconcile',{method:'POST',body:'{}'});toast(`${result.reconciled} sessions reconciled with openNDS`);renderPage('operations');};
}
...
async function renderSettings() {
  ...
  <section class="setting-card payment-setting"><h2>Customer access methods</h2><p>Enable any combination, including Free-Time-only.</p><div class="access-method-switches">${toggle('enableVoucher','Voucher mode','Show code entry and accept generated vouchers',s.paymentMode==='voucher'||s.paymentMode==='both')}${toggle('enableCoin','Coin mode','Show packages and accept confirmed hardware pulses',s.paymentMode==='coin'||s.paymentMode==='both')}${toggle('freeTimeEnabled','Free time','Emphasize the complimentary claim on the captive portal',s.freeTimeEnabled)}</div><div class="field" style="margin-top:14px"><label>Value per hardware pulse (peso)</label><input name="coinPulseValue" type="number" min="1" max="100" value="${s.coinPulseValue||1}"><small>Use 1 for a standard ₱1 pulse acceptor.</small></div>${toggle('requireSignedCoinRequests','Protected coin messages','Require timestamps and persistent monotonic sequences from network coin nodes',s.requireSignedCoinRequests)}</section>
  <section class="setting-card"><h2>Session & pause enforcement</h2><p>Restart-safe time accounting and customer controls</p>${toggle('customerPauseEnabled','Customer pause','Allow customers to pause and resume their own paid session',s.customerPauseEnabled)}${toggle('autoPauseOnDisconnect','Pause when portal heartbeat stops','Optional kiosk policy; keep disabled when customers close the captive-portal window',s.autoPauseOnDisconnect)}<div class="form-grid"><div class="field"><label>Pauses per session</label><input name="pauseLimitCount" type="number" min="0" max="100" value="${s.pauseLimitCount??3}"></div><div class="field"><label>Maximum pause (minutes)</label><input name="maxPauseMinutes" type="number" min="0" max="43200" value="${s.maxPauseMinutes??720}"></div><div class="field"><label>Missing heartbeat (minutes)</label><input name="inactivityPauseMinutes" type="number" min="0" max="1440" value="${s.inactivityPauseMinutes??10}"></div><div class="field"><label>Low-time warning (minutes)</label><input name="lowTimeWarningMinutes" type="number" min="0" max="1440" value="${s.lowTimeWarningMinutes??5}"></div></div></section>
  <section class="setting-card"><h2>Network security</h2><p>Policies used by the router deployment</p>${toggle('clientIsolation','Client isolation policy','Keep customer devices from reaching each other; AP isolation must also be enabled',s.clientIsolation)}<div class="field" style="margin-top:14px"><label>IPv6 policy</label><select name="ipv6Policy"><option value="block">Block until managed</option><option value="managed">Managed dual stack</option><option value="allow">Allow upstream IPv6</option></select><small>Block prevents IPv6 from bypassing IPv4 captive enforcement.</small></div></section>
  <section class="setting-card"><h2>Retention & recovery</h2><p>Audit and backup lifecycle</p><div class="form-grid"><div class="field"><label>Audit retention (days)</label><input name="auditRetentionDays" type="number" min="1" max="3650" value="${s.auditRetentionDays??365}"></div><div class="field"><label>Backup retention (days)</label><input name="backupRetentionDays" type="number" min="1" max="3650" value="${s.backupRetentionDays??30}"></div></div><a class="secondary-btn w-100" href="#operations">Open operational intelligence</a></section>
  <section class="setting-card"><h2>Speed limits</h2><p>Per-user limits are separate from the whole WAN CAKE ceiling.</p><div class="form-grid"><div class="field"><label>Per-user download (Mbps)</label><input name="downloadLimitMbps" type="number" min="1" value="${s.downloadLimitMbps}"></div><div class="field"><label>Per-user upload (Mbps)</label><input name="uploadLimitMbps" type="number" min="1" value="${s.uploadLimitMbps}"></div><div class="field"><label>WAN download ceiling (Mbps)</label><input name="wanDownloadMbps" type="number" min="1" value="${s.wanDownloadMbps||142}"></div><div class="field"><label>WAN upload ceiling (Mbps)</label><input name="wanUploadMbps" type="number" min="1" value="${s.wanUploadMbps||142}"></div></div><small>For a measured 150 Mbps line, 142 Mbps is a practical 95% CAKE starting point. Enter the real upload separately.</small></section>
  ...
  $(`[name="ipv6Policy"]`).value=s.ipv6Policy||'block';
...
async function saveSettings(e){e.preventDefault();const f=new FormData(e.target),voucher=f.get('enableVoucher')==='on',coin=f.get('enableCoin')==='on',mode=voucher&&coin?'both':voucher?'voucher':coin?'coin':'none',body={...state.settings,shopName:f.get('shopName'),timezone:f.get('timezone'),currency:f.get('currency'),portalMessage:f.get('portalMessage'),paymentMode:mode,coinPulseValue:+f.get('coinPulseValue'),buyTime:coin,vouchers:voucher,freeTimeEnabled:f.get('freeTimeEnabled')==='on',customerPauseEnabled:f.get('customerPauseEnabled')==='on',pauseLimitCount:+f.get('pauseLimitCount'),maxPauseMinutes:+f.get('maxPauseMinutes'),inactivityPauseMinutes:+f.get('inactivityPauseMinutes'),autoPauseOnDisconnect:f.get('autoPauseOnDisconnect')==='on',lowTimeWarningMinutes:+f.get('lowTimeWarningMinutes'),requireSignedCoinRequests:f.get('requireSignedCoinRequests')==='on',clientIsolation:f.get('clientIsolation')==='on',ipv6Policy:f.get('ipv6Policy'),auditRetentionDays:+f.get('auditRetentionDays'),backupRetentionDays:+f.get('backupRetentionDays'),downloadLimitMbps:+f.get('downloadLimitMbps'),uploadLimitMbps:+f.get('uploadLimitMbps'),wanDownloadMbps:+f.get('wanDownloadMbps'),wanUploadMbps:+f.get('wanUploadMbps'),maintenanceSchedule:f.get('maintenanceSchedule')==='on'};await saveSettingsObject(body);toast('Production policies and system settings saved');}
...
async function restoreBackup(event){const file=event.target.files?.[0];if(!file)return;try{const backup=JSON.parse(await file.text()),verified=await api('/backup/verify',{method:'POST',body:JSON.stringify(backup)});if(!confirm(`Verified backup from ${verified.createdAt}. Restore it now? A pre-restore recovery snapshot will be created.`))return;await api('/backup/restore',{method:'POST',body:JSON.stringify(backup)});toast('Verified backup restored. Reloading…');setTimeout(()=>location.reload(),600);}catch(error){toast(error.message,'error');}event.target.value='';}
...
document.addEventListener('click', async event => {
  ...
  const session = event.target.closest('[data-session]'); if(session){const action=session.dataset.action;if((action==='stop'||action==='pause')&&!confirm(`${action==='stop'?'Revoke internet access':'Pause this session'} now? The gateway result will be verified.`))return;await api(`/sessions/${session.dataset.session}/${action}`,{method:'POST'});toast(action==='stop'?'Access revoked and verified':`Session ${action} verified by openNDS`);return renderPage('sessions');}
  ...
  const toggleNode=event.target.closest('[data-toggle-node]');if(toggleNode){const enabled=toggleNode.dataset.enabled==='true';await api(`/coin-nodes/${toggleNode.dataset.toggleNode}`,{method:'PUT',body:JSON.stringify({enabled})});toast(`Coin node ${enabled?'enabled':'disabled'}`);return renderPage('coin-nodes');}
