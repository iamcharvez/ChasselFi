...
if ! tc qdisc replace dev "$lan" root cake bandwidth "${download}Mbit" 2>"${result}.stderr"; then
    printf 'error=LAN CAKE failed: %s\n' "$(tr '\n' ' ' <"${result}.stderr")" >"$result"
    rm -f "$request" "${result}.stderr"
    exit 1
fi
if [[ -n "$wan" ]] && ! tc qdisc replace dev "$wan" root cake bandwidth "${upload}Mbit" 2>"${result}.stderr"; then
    printf 'error=WAN CAKE failed: %s\n' "$(tr '\n' ' ' <"${result}.stderr")" >"$result"
    rm -f "$request" "${result}.stderr"
    exit 1
fi
printf 'ok=CAKE applied to LAN %s%s\n' "$lan" "${wan:+ and WAN $wan}" >"$result"
rm -f "$request" "${result}.stderr"
